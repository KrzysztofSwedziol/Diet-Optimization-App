...
public class UnitService {
    ...
    public List<UnitDTO> getAllUnits() {
        return unitRepository.findAll().stream().map(UnitDTO::fromEntity).collect(Collectors.toList());
    }
    ...
    public UnitDTO createUnit(UnitDTO unitDTO) {
        Unit unit = Unit.builder().name(unitDTO.name()).symbol(unitDTO.symbol()).build();
