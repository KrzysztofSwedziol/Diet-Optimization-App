...
public class DataInitializer {
    ...
    private List<Product> initializeProducts(ProductRepository productRepository, Map<String, Unit> units) {
        List<Product> products = List.of(
            new Product(null, "Beef, ground, 90% lean meat / 10% fat, raw", units.get("gram"), 1.0, 185.0, 18.2, 0.0, 12.8, null),
            new Product(null, "Egg, whole, raw, frozen", units.get("piece"), 50.0, 150.0, 12.3, 0.91, 10.3, null),
            new Product(null, "Potatoes, gold, without skin, raw", units.get("gram"), 1.0, 73.5, 1.81, 16.0, 0.264, null),
            new Product(null, "Rice, white, long grain, unenriched, raw", units.get("gram"), 1.0, 359.0, 7.04, 80.3, 1.03, null),
            ...
            new Product(null, "Bananas, ripe and slightly ripe, raw", units.get("piece"), 115.0, 97.0, 0.74, 23.0, 0.29, null)
        );
    ...
    private void initializePreferences(ProductPreferenceRepository preferenceRepository, User user, List<Product> products) {
        ...
        List<ProductPreference> preferences = products.stream()
            .map(product -> {
                ...
                return ProductPreference.builder()
                    ...
                    .owner(user)
                    .product(product)
                    .preference(random.nextDouble() * 10) // preference between 0.0 and 10.0
                    .build();
            })
            .toList();
